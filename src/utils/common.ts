...
export const deepClone = function (data: any): any {
    var objClone: any = Array.isArray(data) ? [] : {};
...
@param { String } key 排序的参数
 @return
 */
export const sortArray = <T>(list: Array<T>, key: string, defaultVal?: string | number): Array<T> => {
    if (list.length <= 1) {
        return list
    }
    ...
    return sortArray(preList, key, defaultVal).concat(currList, sortArray(nextList, key, defaultVal))
}
...
*/
export const is_weixin = ():boolean=> {
